...
goToBackBtn.addEventListener('click', backToGetUsersLaundryState);
...
function handleSubmitReviewForm() {
    ...
    // const urlParams = new URL(location.href).searchParams;
    // const UserId = urlParams.get('userId');
    const userId = '12345UserId';
    // formData.append('reviewUserId', UserId);

    const laundryId = '12345laundryId';

    $.ajax({
        type: 'POST',
        url: `/api/userId/laundryId/review`, // 추후 수정 `/api/${userId}/${laundryId}/review`
        data: formData,
        // async: false,
        processData: false,
        contentType: false,
